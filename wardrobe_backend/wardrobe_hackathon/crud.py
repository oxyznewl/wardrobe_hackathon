...
def stats_by_category(start: date, end: date):
    db = SessionLocal()
    j = wear_log_item.join(Clothes, wear_log_item.c.clothes_id == Clothes.id)\
                     .join(WearLog, wear_log_item.c.wear_log_id == WearLog.id)
    ...
    stmt = select(Clothes.category, func.count(wear_log_item.c.clothes_id).label("cnt"))\
        .select_from(j)\
        .where(WearLog.date.between(start, end))\
...
# 2. 계절별(봄/여름...) 통계
def stats_by_season(start: date, end: date):
    db = SessionLocal()
    j = wear_log_item.join(Clothes, wear_log_item.c.clothes_id == Clothes.id)\
                     .join(WearLog, wear_log_item.c.wear_log_id == WearLog.id)
    
    # 계절 컬럼(seasons) 기준으로 그룹핑
    stmt = select(Clothes.seasons, func.count(wear_log_item.c.clothes_id).label("cnt"))\
        .select_from(j)\
        .where(WearLog.date.between(start, end))\
        .group_by(Clothes.seasons)
    
    rows = db.execute(stmt).all()
    db.close()
    # 프론트엔드에서 'category'라는 키를 쓰거나, 'label'을 쓰는데 
    # 통일성을 위해 여기도 키 이름을 맞추거나 프론트에서 처리해야 함.
    # 아까 리액트 코드에 맞춰서 리턴합니다.
